...
float4 g_materialDiffuseColor;		//マテリアルのディフューズカラー。
 ...
 *@brief	頂点シェーダー。
 */
float4 PSMain( VS_OUTPUT In, uniform bool hasTex ) : COLOR
{
	//ライトを計算。
	float4 lig = 0.0f;
	{
	...
	float4 color = 0.0f;
	if( hasTex ){
		color = tex2D( g_diffuseTextureSampler, In.uv );
	}else{
